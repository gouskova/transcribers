...
with open(os.path.join(phonopath, 'wds_t_s_tsh/LearningData.txt'), 'w', encoding='utf-8') as f:
    for word in sorted(celexdic):
        f.write(celexdic[word]['IPA'].replace('tʃ', 'ʧ').replace('dʒ', 'ʤ').strip()+'\n')

with open(os.path.join(phonopath, 'wds_ts_tsh/LearningData.txt'), 'w', encoding='utf-8') as f:
    for word in sorted(celexdic):
        f.write(celexdic[word]['IPA'].replace('tʃ', 'ʧ').replace('dʒ', 'ʤ').replace('t s', 'ʦ').replace('d z', 'ʣ').strip()+'\n')

        
with open(os.path.join(phonopath, 'wds_t_s_t_sh/LearingData.txt'), 'w', encoding='utf-8') as f:
    for word in sorted(celexdic):
...
with open(os.path.join(phonopath, 'wds_ts_t_sh/LearningData.txt'), 'w', encoding='utf-8') as f:
    for word in sorted(celexdic):
        f.write(celexdic[word]['IPA'].replace('tʃ', 't ʃ').replace('dʒ', 'd ʒ').replace('t s', 'ʦ').replace('d z', 'ʣ').strip()+'\n')
